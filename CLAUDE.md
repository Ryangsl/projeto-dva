...
| 2026-09-22 | **Scripts `import:fipe:*` migrados da API FIPE v1 (`parallelum.com.br/fipe/api/v1`) para a v2 (`fipe.parallelum.com.br/api/v2`)** | O script Python original que alimentava a lógica de marcas/modelos foi perdido; o único vestígio no `.env` era `FIPE_API_KEY` apontando (por engano) para a própria URL da v2, enviada como `Authorization: Bearer` — a v1 rejeita qualquer header de auth com 401, então o import estava sempre falhando. Verificado ao vivo que a v2 é pública e não exige token (500 req/dia; um token opcional em `X-Subscription-Token`, não Bearer, eleva a 1000/dia) — trocada a base da API, mantido tudo o resto (`INSERT IGNORE`, sem filtro de marca no import: as 7 marcas do grupo continuam sendo só um filtro de **exibição** em `veiculos.service.ts`, não uma limitação do catálogo importado) |
| 2026-09-22 | **`npm run db:setup` passou a criar `vehicle_brands`/`vehicle_models` (`CREATE TABLE IF NOT EXISTS`) no banco antigo (`painel_procar`), e os scripts `import:fipe:*` passaram a gravar nelas por nome totalmente qualificado (`` \`painel_procar\`.vehicle_brands ``)** | Segundo bug encontrado ao validar a correção da v2 acima: mesmo com a API certa, o import continuava falhando com "Table 'dva_veiculos.vehicle_brands' doesn't exist" — as tabelas nunca foram criadas por `db:setup` (só existem em produção porque o projeto anterior, já removido do código, as criou lá) e, mesmo quando criadas manualmente, os `INSERT`/`SELECT` dos scripts usavam nome de tabela sem qualificar o banco, resolvendo contra `dva_veiculos` (o banco do `pool` compartilhado) em vez de `painel_procar`. Corrigido nos dois pontos: criação idempotente e aditiva em `setup.ts` (nunca altera/apaga nada do banco antigo) + qualificação `` `${VDB}`.tabela `` nos scripts, mesmo padrão já usado em `veiculos.service.ts`. Validado ao vivo: 107 marcas e 7366 modelos importados, incluindo as 7 marcas do grupo |

## 10. Log de Evolução / Changelog

- **2026-09-22** — **Correção dos scripts `import:fipe:marcas`/`import:fipe:anos`: migração para a API FIPE v2.**
  - **Backend**: `database/import-fipe/fipe-client.ts` reescrito para `https://fipe.parallelum.com.br/api/v2/cars` (era a v1, `parallelum.com.br/fipe/api/v1/carros`); `exigirFipeApiKey()` (lançava erro se ausente) virou `obterFipeSubscriptionToken()` (opcional, sem lançar) — o token, quando presente, vai no header `X-Subscription-Token`, nunca mais como `Authorization: Bearer` (a v2 responde 401 a isso). `env.fipe.apiKey`/`FIPE_API_KEY` renomeados para `env.fipe.subscriptionToken`/`FIPE_SUBSCRIPTION_TOKEN` em `config/env.ts`, `.env.example`, `.env` local, `DEPLOY.md`, `DEPLOY-RAPIDO.md`. `marcas-modelos.ts`/`anos.ts` atualizados para o novo shape de resposta da v2 (`{code, name}` em vez de `{codigo, nome}`; `/brands/{id}/models` devolve array direto, sem o wrapper `{modelos: [...]}` da v1).
  - `marcas-modelos.ts` continua importando **todas** as marcas que a FIPE devolver (nenhum filtro por `MARCAS_DVA`) — comentário novo no topo do arquivo deixa explícito que o filtro das 7 marcas do grupo é só de exibição, em `veiculos.service.ts`.
  - Causa raiz confirmada: o `.env` local tinha `FIPE_API_KEY` preenchida com a URL da v2 (não uma chave), enviada como Bearer para a v1 — 401 garantido em toda chamada. Corrigido (variável renomeada e esvaziada; token agora é genuinamente opcional).
  - **Segundo bug, achado ao validar o primeiro**: `db:setup` nunca criou `vehicle_brands`/`vehicle_models` (só existiam em produção por herança do projeto anterior, já removido do código) e, mesmo criadas, os scripts gravavam sem qualificar o banco (`INSERT INTO vehicle_brands` resolvia contra `dva_veiculos`, do `pool` compartilhado, não `painel_procar`). Corrigido: `setup.ts` ganhou `garantirTabelasVeiculosFipe()` (`CREATE TABLE IF NOT EXISTS`, aditivo, roda antes do schema do DVA) e `marcas-modelos.ts`/`anos.ts` passaram a usar `` `${VDB}`.vehicle_brands `` etc., mesmo padrão de `veiculos.service.ts`.
  - Testado ao vivo, ponta a ponta: `npm run db:setup` cria as tabelas em `painel_procar`; `npm run import:fipe:marcas` importa 107 marcas e 7366 modelos sem erro, incluindo as 7 do grupo (RAM, BYD, Denza, Jeep, Mercedes-Benz, Dodge, Chrysler) — confirmado por consulta direta ao banco.
  - `IMPORTACAO-FIPE.md` atualizado (token opcional, sem chave obrigatória, limites 500/1000 por dia; §4 corrigida para descrever a criação automática real).
  - Backend (`npm run typecheck` / `npm run build`) OK.
